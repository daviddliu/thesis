...
Suppose that each clone emits variant reads according to a binomial distribution. Thus, for cluster $k$ and some reads $d_{mn}$, we have variant reads distributed with $\mathrm{Bin}(d_{mn}, \phi_{kz_{nk}})$. We can consider the joint probability of a site by the product of each sample, since we assume they are independent. That is,
\begin{equation}
...
$H \sim U(0,1) \sim \mathrm{Beta}(1, 1)$ = Base distribution for parameters $\phi_{nk}$	 \\
...
$d_{mn} \sim \mathrm{Pois}(\text{Read depth})$ = Observed total reads
\end{quote}
\vspace{0.05cm}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item Draw $z_n | \{\pi_1, \pi_2, \ldots\} \sim \mathrm{Cat}(\pi(\mathbf{w}))$.
...
\begin{align*}
\mathcal{L}_{\text{Entropy}} = - \sum_{n=1}^N \hat{r}_{nk} \log \hat{r}_{nk}
\end{align*}

\subsubsection{Initialization}
Done with k-means++. The responsibilities are initialized with the assignments. For the other parameters, we assume that they are set to their uniform values.
...
$\rightarrow$ As input to phylogeny algorithms
